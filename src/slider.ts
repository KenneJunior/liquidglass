 ...
 */
function charDirection(newChar: string, oldChar: string, globalDir: number): number {
...
export class LiquidGlassSlider {
     ...
     */
    private _updateDigits(newText: string, oldText: string, globalDir: number): void {
        if (!this.digitsEl) return;
    ...
    private _animateDrum(
        ...
        const transition = `transform ${DRUM_DUR}ms ${DRUM_EASE}, opacity ${DRUM_DUR * 0.5}ms ease`;
    ...
    private _updatePosition(): void {
        const {
            trackWidth, thumbWidth, thumbHeight, trackHeight,
            labelPosition, labelSticky,
        } = this.cfg;
    ...
    private _loop(): void {
        ...
        const cloneEl = this.thumb.querySelector<HTMLElement>('.lg-slider-thumb-clone');
