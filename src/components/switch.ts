...
export class LiquidGlassSwitch {
  ...
  constructor(private readonly container: HTMLElement, options: SwitchOptions) {
    this.cfg = {
      ...
      colorOn: toRGBA(options.colorOn, [139, 92, 246, 0.5]),
          ...
          ">
            <div class="lg-switch-thumb" style="
              ...
              transform:translateY(-50%) scale(${
                this.cfg.thumbHeight / this.cfg.thumbWidth
              });
  ...
  private _computeGeo(): void {
    ...
    const tr =
      trackWidth - trackHeight - (thumbWidth - thumbHeight) * restScale;
  ...
  private _loop(): void {
    ...
    this.spTc.setTarget(
      this.isPressed ? (this.thumbRatio > 0.5 ? 1 : 0) : this.checked ? 1 : 0
    );
    ...
    this.iconOffEl.style.transform = `scale(${(0.8 + 0.2 * (1 - tc)).toFixed(
      3
    )})`;
